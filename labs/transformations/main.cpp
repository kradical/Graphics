    ...
    int fbwidth, int fbheight, u8vec4* framebuffer, float* depthbuffer)
{
    ...
    for (int i = 0; i < 3; i++)
    {
        ...
        window_xy[i].x = (glm::int64_t)((+clip_verts[i].x * one_over_w[i] + 1.0f) / 2.0f * (glm::int64_t)fbwidth * 256);
        window_xy[i].y = (glm::int64_t)((-clip_verts[i].y * one_over_w[i] + 1.0f) / 2.0f * (glm::int64_t)fbheight * 256);
    ...
    glm::int64_t triarea2_64 = ((window_xy[2].x - window_xy[0].x) * (window_xy[1].y - window_xy[0].y) - (window_xy[2].y - window_xy[0].y) * (window_xy[1].x - window_xy[0].x));
    ...
    for (int v = 0; v < 3; v++)
    {
        ...
        const glm::int64_t s168_zero_pt_five = 0x80;
    ...
    for (glm::int64_t y = window_bbox_min.y; y <= window_bbox_max.y; y++)
    {
        i64vec3 row_bary = bary;

        for (glm::int64_t x = window_bbox_min.x; x <= window_bbox_max.x; x++)
        {
    ...
    int fbwidth, int fbheight, u8vec4* framebuffer, float* depthbuffer)
{
    ...
    ////////////////////
    mat4 scene = rotate(curr_time, vec3(0, 1, 0));

    float amount = std::abs(std::fmod(curr_time, 4) - 2);
    ...
    // rotated cube
    {
        ...
        ////////////////////
        mat4 model = rotate(curr_time, vec3(0, 0, 1));

        mat4 modelviewproj = proj * view * scene * model;
    ...
    // translated cube
    {
        ...
        ////////////////////
        mat4 model = translate(vec3(3, 0, amount));
    ...
    // scaled cube
    {
        ...
        ////////////////////
        mat4 model = translate(vec3(-3, 0, 0)) * scale(vec3(amount));
